...
const TasksControllersAndSelectors = ({
	...
	tabSelector: "Stats" | "Tasks" | "Favourite Tasks";
	setTabSelector: React.Dispatch<
		React.SetStateAction<"Stats" | "Tasks" | "Favourite Tasks">
	>;
	locationPathname: string;
	folderID?: string;
	projectID?: string;
	taskPriority: "All" | TTaskPriority;
}): ReactNode => (
	<div className="flex justify-between flex-wrap gap-4">
		<div className="flex items-center overflow-auto pl-0 md:pl-10">
			<button
				className={`flex items-center gap-2 p-2 cursor-pointer transition-all duration-300 relative ${
					tabSelector == "Stats" ? "" : "opacity-80 hover:opacity-95"
				}`}
				onClick={() => setTabSelector("Stats")}
			>
				<span
					className={`bg-white h-0.5 left-0 bottom-0 absolute w-0 transition-all duration-300 ${
						tabSelector == "Stats" ? "w-full" : ""
					}`}
				></span>
			...
			>
				...
				></span>
				<LuClipboardList className="text-lg flex-none" />{" "}
				<span className="text-base font-semibold w-fit flex-none">
					All Tasks
				</span>
			</button>
			<button
			...
			>
				...
				></span>
				<LuStar className="text-lg flex-none" />{" "}
				<span className="text-base font-semibold flex-none">
					Favourite Tasks
				</span>
			</button>
		</div>
...
const TasksStats = ({
	...
	tasks: ITask[];
	dataCat: TasksProjectsCatWithColorStats[];
	dataPrios: TasksProjectsCatWithColorStats[];
}): ReactNode => (
	<div className="flex flex-col gap-2 min-w-[300px] flex-1 max-w-[645px]">
		<div className="flex gap-2 flex-wrap">
			<div className="glass flex flex-col p-3 rounded-2xl gap-2 px-2 w-full sm:w-auto">
				<h2 className="text-left w-full font-semibold">Tasks Status</h2>
				<PieChartContainer data={dataCat} />
			</div>
			<div className="glass flex flex-col p-3 rounded-2xl gap-2 px-2 w-full sm:w-auto">
				<h2 className="text-left w-full font-semibold">Tasks Priorities</h2>
				<PieChartContainer data={dataPrios} />
			</div>
		</div>
...
export default function TasksHolderBody({
	tasks,
	navigate,
	setPrevPath,
	folderID,
